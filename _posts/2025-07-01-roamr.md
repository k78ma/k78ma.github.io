...
Our synthesized goals for the project were thus to build an AMR platform that is:
...
We first came up with some concept sketches (one is shown below) of how we wanted it to look and function mechanically. The general idea was to have a 2-wheel differential drive configuration, with a ball caster for stability; this would let the robot be user-friendly (simple kinematics to program) and minimize the number of motors, keeping the cost low.
...
### roamr v1 prototype
The concepts that we came up with were evaluated with respect to our goals, constraints, and requirements. The winning concept, with slight tweaking to make everything work more smoothly, was implemented in CAD.
...
- **Phone stand:** Instead of the awkward fixed clamp, we added a sliding mechanism that allowed for adjustable clamping with much better ease-of-use. 
- **Phone configuration:** Originally, we had the phone horizontally, as this provided a wider field of view. However, we realized that both the horizontal and vertical configurations had relatively narrow field of view, and either way we didn't have 360-degree LiDAR; thus, both configurations would need to turn to do a 360 scan. The vertical configuration allows for easier access to the phone app screen and reduces the robot size, so we switched.
...
- **Chassis:** The original chassis didn't protect the battery (in fact, we just secured it with duct tape). The final design was expanded to both protect the battery and also allow easier access to electronics, along with more mounting holes for flexibility and extensibility.
- **Battery mount:** We added a full top and bottom battery mount to hold the battery securely but also allow it to be removed easily (sliding lock mechanism) for recharging.
...
As part of the roamr v1 integration test, we realized the hardware design with off-the-shelf components (power regulator modules, ESP32 development modules, and motor driver modules) and hand-soldered wires to create a rapid prototype. After confirming that the basic design was valid (able to control motors as desired), we moved on to refining it for the final design.
...
### iOS App
The roamr iOS app, shown below, serves as the primary interface for accessing sensor data and running code. It coordinates with the ESP32 microcontroller to write motor commands and read encoder feedback, and executes sensor/robot processing logic via an embedded WebAssembly (WASM) runtime, and providing real-time visualization and teleoperation capabilities through a websocket-based interface.
...
<figure class="mt-3 mb-3">
  <video class="w-100 rounded shadow" controls loop playsinline preload="metadata">
    <source src="/assets/video/roamr-demo-fast.mp4" type="video/mp4">
    Your browser does not support the video tag.
  </video>
  <figcaption class="caption">roamr demo: autonomous mapping and navigation (sped-up)</figcaption>
</figure>
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.html path="assets/img/roamr-state-diagram.png" class="img-fluid rounded z-depth-1" %}
    </div>
</div>
<div class="caption">
     State diagram of autonomy demo components.
</div>
...
- *Implementing a proper SLAM system on roamr.* The above autonomy demo is not doing actual loop-closure and just relying on odometry for localization.
- *Legs? Arms?* The hardware is architected such that other components should be easy to add, but we never got to try adding more stuff. This could also be sensors, etc.
- *End-to-end NN driving and fleets.* [OpenBot](https://www.openbot.org/research), another phone-based robot, showcased some interesting projects using end-to-end neural networks to drive, as well as doing collective learning with fleets. Why not try the same with roamr?